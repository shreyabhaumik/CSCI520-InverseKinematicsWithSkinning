...
static double allLightsIntensity = 0.4;
...
int IKalgo = 0, SkinningMethod = 0; // to provide options for IK algorithms and Skinning methods
...
enum {SKY_NEGX=0,SKY_POSX,SKY_POSY,SKY_NEGY,SKY_POSZ,SKY_NEGZ};

// For automation of a particular sequence of handle drag motion
bool automateDrag = false;
int autoCount = 0;
...
// To load a texture image into skybox
unsigned int loadTexIntoSkybox(char* cubemapFile)
{
    unsigned int textureID;
    glGenTextures(1, &textureID);
    Pic* texImage = loadTexImage(cubemapFile);
...
void initSkybox()
{
  skybox[SKY_POSX] = loadTexIntoSkybox((char*)"../skybox/posx.ppm");
  skybox[SKY_NEGX] = loadTexIntoSkybox((char*)"../skybox/negx.ppm");
  skybox[SKY_POSY] = loadTexIntoSkybox((char*)"../skybox/posy.ppm");
  skybox[SKY_NEGY] = loadTexIntoSkybox((char*)"../skybox/negy.ppm");
  skybox[SKY_POSZ] = loadTexIntoSkybox((char*)"../skybox/posz.ppm");
  skybox[SKY_NEGZ] = loadTexIntoSkybox((char*)"../skybox/negz.ppm");
}

// To show skybox
void showSkybox()
{
  int s = 20 * modelRadius;
...
static void updateSkinnedMesh()
{
  ...
  skinning->applySkinning(fk->getJointSkinTransforms(), newPosv, fk, SkinningMethod); // added FK * fk to do Dual Quaternion Skinning
...
static void idleFunction()
{
  ...
  // automating a particular sequence of handle drag
  ...
  if(automateDrag)
  {
    ...
    if(autoCount < 150)
    {
      myPosDiff = Vec3d(-maxOneStepDistance, 0.0, 0.0);
      IKJointPos[0] += myPosDiff;
      ...
      handleControl.setMouseButtonActivity(true, true, false,
          IKJointPos[0], 0.0f, getClosestHandle, addOrRemoveHandle);
    }
    else if(autoCount < 300)
    {
      myPosDiff = Vec3d(0.0, maxOneStepDistance, 0.0);
      IKJointPos[1] += myPosDiff;
      auto getClosestHandle = [&]() -> int
      {
        return 1;
      };
      ...
      handleControl.setMouseButtonActivity(true, true, false,
          IKJointPos[1], 0.0f, getClosestHandle, addOrRemoveHandle);
    }
    else if(autoCount < 400)
    {
      myPosDiff = Vec3d(maxOneStepDistance, 0.0, 0.0);
      IKJointPos[2] += myPosDiff;
      auto getClosestHandle = [&]() -> int
      {
        return 2;
      };
      ...
      handleControl.setMouseButtonActivity(true, true, false,
          IKJointPos[2], 0.0f, getClosestHandle, addOrRemoveHandle);
    }
    else if(autoCount < 550)
    {
      myPosDiff = Vec3d(0.0, 0.0, 2.0*maxOneStepDistance);
      IKJointPos[2] += myPosDiff;
      auto getClosestHandle = [&]() -> int
      {
        return 2;
  ...
  ik->doIK(IKJointPos.data(), fk->getJointEulerAngles(), IKalgo);
  ...
  if (elapsedTime >= 1.0 / 4)
  {
    ...
    const char * IKa = (IKalgo == 2)? "Transpose" : ((IKalgo == 1)? "Pseudoinverse" : "Tikhonov");
    const char * SkinM = (SkinningMethod == 1)? "DQS" : "LBS";
    sprintf(windowTitle, "Vertices: %d | %.1f FPS | graphicsFrame %d | IK Algo: %s | Skinning Method: %s ", meshDeformable->Getn(), fpsBuffer.getAverage(), graphicsFrameID, IKa, SkinM);
    glutSetWindowTitle(windowTitle);
    titleBarFrameCounter = 0;
  }
  graphicsFrameID++;

  // To save the 550 screenshots to disk
  if ((((shotCount) >= 0) && ((shotCount) < 550)) && (takeShots))
  {
    char filenum[4];
    sprintf(filenum, "%03d", (shotCount));
    std::string filename1("../Animations/pic" + std::string(filenum) + ".ppm");
    char *s = new char[filename1.size() + 1];
    strcpy(s, filename1.c_str());
    saveScreenshot(s);
    shotCount++;
  }
  else if ((shotCount) >= 550)
    takeShots = false;
...
//    glDrawBuffer(GL_BACK);
  ...
  // While making the star and it's files, I realised the different objects used will have different sizes and it would be nice to
  // know their relative sizes... That way we can understand also, how much pull on an IK handle is too much. That's why placed the
  // utah teapot for comparison of sizes.
  glEnable(GL_LIGHTING);
  glColorMaterial (GL_FRONT_AND_BACK, GL_AMBIENT) ;
  glEnable(GL_COLOR_MATERIAL);
  glColor3f(0.0648,0.01718,0.01679);
  glPushMatrix();
...
static void keyboardFunc(unsigned char key, int x, int y)
{
  switch (key)
  {
    ...
    case 'x': // To take 550 screenshots
      takeShots = true;
      break;
    ...
    case 'r': // run automation of a particular sequence of handle grag
      automateDrag = !automateDrag;
      if(!automateDrag)
      {
        resetSkinningToRest();
        // camera->Reset();
...
static void initialize()
{
  ...
  lighting->SetAllLightsEnabled(true);
  // lighting->SetLightEnabled(6, true);
  // lighting->SetLightEnabled(3, true);
  // lighting->SetLightEnabled(6, true);
  // lighting->SetLightEnabled(5, true);
  // lighting->SetLightEnabled(8, true);
  lighting->SetLightIntensity(0, 0.3);
  lighting->SetLightIntensity(1, 0.1);
  lighting->SetLightIntensity(2, 0.1);
  lighting->SetLightIntensity(3, 0.3);
  lighting->SetLightIntensity(4, 0.1);
  lighting->SetLightIntensity(5, 0.3);
  lighting->SetLightIntensity(6, 0.3);
  lighting->SetLightIntensity(7, 0.1);
  ...
  //   clear to white - I changed to a shade of light blue
  glClearColor(180.0 / 256, 206.0 / 256, 232.0 / 256, 0.0);
...
int main (int argc, char ** argv)
{
  ...
  if ( argc < numFixedArgs )
  {
    ...
    cout << "The next two arguments are optional:" << endl;
    cout << "Next argument is for IK algorithm - 0(Tikhonov IK method), 1(Pseudoinverse method), 2(Transpose method)" << endl;
    cout << "Next argument is for Skinning method - 0(Linear Blend Skinning), 1(Dual Quaternion Skinning)" << endl;
    cout << "By default both are set to 0." << endl;
    return 0;
  }
